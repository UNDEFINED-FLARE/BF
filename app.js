...
if (auth && currentUser && !currentUser.uid.startsWith('guest_')) await auth.signOut();
if (feedListener && database) database.ref('posts').off('value', feedListener);
if (qaListener && database) database.ref('posts').off('value', qaListener);
if (newsListener && database) database.ref('posts').off('value', newsListener);
currentUser = null;
...
// Only set auth state listener for non-guest users
...
if (!database) {  
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"><i class="fas fa-exclamation-triangle"></i></div><h3>Database not connected</h3><p>Please refresh the page.</p></div>';  
    return;  
}  
...
feedListener = postsRef.on('value', (snapshot) => {  
    ...
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"><i class="fas fa-exclamation-circle"></i></div><h3>Error loading posts</h3><p>Please refresh.</p></div>';  
});
...
if (currentFilter === 'latest') {  
    ...
    container.innerHTML = `<div class="empty-state"><div class="empty-icon"><i class="fas fa-users"></i></div><h3>Following Feed</h3><p>Follow users to see their posts here.</p></div>`;  
    return;  
}  
  
if (displayPosts.length === 0) {  
    container.innerHTML = `<div class="empty-state"><div class="empty-icon"><i class="fas fa-inbox"></i></div><h3>No posts yet</h3><button onclick="openComposer()" style="margin-top:15px;padding:10px 20px;background:#0B1F3A;color:white;border:none;border-radius:8px;cursor:pointer;"><i class="fas fa-plus"></i> Create Post</button></div>`;  
} else {  
...
if (!database) {  
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"><i class="fas fa-database"></i></div><h3>Database not connected</h3></div>';  
    return;  
}  
...
qaListener = postsRef.on('value', (snapshot) => {  
    ...
    if (qaPosts.length === 0) {  
        container.innerHTML = `<div class="empty-state"><div class="empty-icon"><i class="fas fa-question-circle"></i></div><h3>No questions yet</h3><button onclick="openComposerAndSetQuestion()" style="margin-top:15px;padding:10px 20px;background:#0B1F3A;color:white;border:none;border-radius:8px;cursor:pointer;"><i class="fas fa-pen"></i> Ask a Question</button></div>`;  
    } else {  
...
if (!database) {  
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"><i class="fas fa-newspaper"></i></div><h3>Database not connected</h3></div>';  
    return;  
}  
...
newsListener = postsRef.on('value', (snapshot) => {  
    ...
    if (newsPosts.length === 0) {  
        container.innerHTML = `<div class="empty-state"><div class="empty-icon"><i class="fas fa-bullhorn"></i></div><h3>No announcements yet</h3></div>`;  
    } else {  
...
if (myPosts.length === 0) {  
    container.innerHTML = `<div class="empty-state"><div class="empty-icon"><i class="fas fa-edit"></i></div><h3>No posts yet</h3></div>`;  
} else {  
...
const catIcons = { general: 'fa-comment', question: 'fa-question-circle', announcement: 'fa-bullhorn', event: 'fa-calendar-alt', resource: 'fa-book' };  
const catIcon = catIcons[post.category] || 'fa-comment';  
const catBadge = `<span class="post-category-badge cat-${post.category || 'general'}"><i class="fas ${catIcon}"></i></span>`;  

const hashtagsHtml = (post.hashtags || []).map(h => `<span class="hashtag"><i class="fas fa-hashtag"></i>${escapeHtml(h.substring(1))}</span>`).join('');  
        ...
        post.poll.options.forEach((opt, idx) => {  
            ...
            pollHtml += `  
                <div class="poll-option ${isSelected ? 'selected' : ''}" data-option-index="${idx}" onclick="event.stopPropagation();votePoll('${post.id}', ${idx})">  
                    ...
                    <span class="poll-label"><i class="fas fa-poll-h"></i> ${escapeHtml(opt.label)}</span>  
                    <span class="poll-pct">${pct}% (${voteCount})</span>  
                </div>  
            `;  
        });  
        pollHtml += `<p class="poll-footer"><i class="fas fa-check-circle"></i> ${hasVoted ? 'You voted' : 'Tap to vote'} · ${total} total vote${total !== 1 ? 's' : ''}</p></div>`;  
    } catch(e) {  
        console.error('Poll render error:', e);  
        pollHtml = '<div class="poll-container"><p style="color:red;font-size:12px"><i class="fas fa-exclamation-triangle"></i> Poll error</p></div>';  
    }  
}  

const answerButton = (post.category === 'question') ?   
    `<button class="action-btn" onclick="event.stopPropagation();openAnswerModal('${post.id}')"><i class="fas fa-reply"></i> Answer</button>` : '';  
...
card.innerHTML = `  
    <div class="post-header">  
        ...
        <div class="post-meta">  
            <div class="post-name">  
                ...
                ${isOfficial && !post.anonymous ? '<span class="verified-badge"><i class="fas fa-check-circle"></i></span>' : ''}  
            </div>  
            <div class="post-time"><i class="far fa-clock"></i> ${time}</div>  
        </div>  
        ${catBadge}  
    </div>  
    ...
    <div class="post-actions">  
        <button class="action-btn ${isLiked ? 'liked' : ''}" onclick="event.stopPropagation();toggleLike('${post.id}',this)">  
            <i class="fas ${isLiked ? 'fa-heart' : 'fa-heart'}"></i> ${likeCount || ''}  
        </button>  
...
if (preview) preview.innerHTML = hashtags.map(h => `<span class="hashtag"><i class="fas fa-hashtag"></i>${escapeHtml(h.substring(1))}</span>`).join('');
...
try {  
    ...
    commentsDiv.innerHTML = '<p class="comments-label"><i class="fas fa-comments"></i> Replies</p>';  
    ...
    if (commentsData) {  
        ...
        commentsDiv.innerHTML += '<div class="empty-state"><p><i class="far fa-comment-dots"></i> No replies yet. Be the first!</p></div>';  
    }  
...
div.innerHTML = `<div class="comment-avatar" style="${isOfficial ? 'background:var(--green)' : ''}">${comment.anonymous ? '<i class="fas fa-user-secret"></i>' : getInitials(comment.displayName || 'U')}</div><div class="comment-bubble"><div class="comment-author">${escapeHtml(comment.displayName || 'Unknown')}</div><div class="comment-text">${escapeHtml(comment.text || '')}</div><div class="comment-time"><i class="far fa-clock"></i> ${formatTime(comment.createdAt)}</div></div>`;
return div;
}
